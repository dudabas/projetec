...
$dia = "sexta-feira"; // Alterado para sexta-feira

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $controller->salvar($dia);
    header("Location: cardapioadmsexta.php?salvo=1");
    exit;
}
...
<main class="container py-5">
  <h1 class="cardapiodia-page-title mb-4">Sexta-feira</h1> 
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <div class="text-center mb-4">
      ...
      <label for="fileInput" class="btn btn-primary mt-2 w-100">
          Escolher imagem
      </label>

      <input type="file" name="imagem" accept="image/*" id="fileInput" style="display:none;">
  </div>
...
<script>
  document.querySelectorAll('.editar-btn').forEach(btn => {
    btn.onclick = () => {
      ...
      let lista = card.querySelector('ul');
      let textarea = card.querySelector('textarea');

      lista.style.display = 'none';
      textarea.style.display = 'block';

      textarea.value = [...lista.querySelectorAll('li')]
